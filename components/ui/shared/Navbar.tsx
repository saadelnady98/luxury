...
const Navbar = () => {
  ...
  return (
    <div className="bg-transparent text-mainColor relative top-0 left-0 right-0 z-40">
      ...
      >
        <div className="h-14 flex justify-center items-center  text-xl ">
          <Link href={`/${lang}`}>
            <Image
              className="h-14 w-14"
              src={images.Logo}
            ...
            {navCategories?.map((category) =>
              category.slug !== "sell" && category.slug !== "rent" ? (
                <li
                  key={category?.id}
                  className={`hover:text-textColor transition text-sm xl:text-base ${
                    pathname.endsWith(`/communities/${category.slug}`) &&
                    "text-textColor"
                  }`}
                >
                  <Link href={`/${lang}/communities/${category.slug}`}>
                    {category.name}
                  </Link>
                </li>
              ) : null
      ...
      >
        ...
        <p className="text-mainGray text-sm py-4 border-b-[1px] border-subDark">
          Main Categories
        </p>
